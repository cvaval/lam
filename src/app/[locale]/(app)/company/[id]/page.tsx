  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <header className="rounded-2xl border border-chabon/10 bg-white p-6">
        ...
        <h1 className="mt-2 font-serif text-3xl font-semibold text-ank">{company.name}</h1>
      ...
      <section>
        <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-ank/80">{t.doc.publications}</h2>
            ...
            const inner = (
              <div className="flex items-center justify-between rounded-xl border border-chabon/10 bg-white px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium text-ank">{p.label}</p>
                ...
                {p.date && <span className="text-xs text-ank/80">{formatDate(locale, p.date)}</span>}
              </div>
            )
            return p.document ? (
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <dt className="text-[11px] uppercase tracking-wide text-ank/80">{label}</dt>
      <dd className="text-ank">{value}</dd>
    </div>
  )
}
