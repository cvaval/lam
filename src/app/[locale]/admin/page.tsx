...
export default async function AdminOverview({ params }: { params: { locale: string } }) {
  ...
  // ⚠️ CHAQUE CHIFFRE EST UNE PORTE. Un compteur qu'on ne peut pas ouvrir est une affirmation
  // qu'on ne peut pas vérifier : « 6 alertes scraping » — de qui ? « 9 inscrits » — lesquels ?
  // Demande de la rédaction du 16 sept. 2026. Chaque carte mène à la liste qu'elle résume.
  const kpis = [
    { label: t.admin.kpiUsers, value: registered, note: null, alerte: false, href: `/${locale}/admin/users` },
    { label: t.admin.kpiSearches, value: searchesToday, note: null, alerte: false, href: `/${locale}/admin/recherches` },
    { label: t.admin.kpiScraping, value: scrapingAlerts, note: null, alerte: false, href: `/${locale}/admin/logs?action=SCRAPING_ALERT&jours=0` },
    {
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {kpis.map((k) => (
          <Link
            key={k.label}
            href={k.href}
            className={`block rounded-2xl border bg-white p-5 transition hover:border-chabon focus-visible:outline focus-visible:outline-2 focus-visible:outline-chabon ${k.alerte ? 'border-wouj' : 'border-chabon/10'}`}
          >
            <p className={`font-mono text-4xl font-semibold tracking-tight ${k.alerte ? 'text-wouj' : 'text-ank'}`}>
              {k.value.toLocaleString('fr')}
            </p>
            <p className="mt-1 text-xs uppercase tracking-wide text-ank/80">{k.label} <span aria-hidden className="text-chabon">→</span></p>
