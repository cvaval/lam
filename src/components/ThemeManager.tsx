...
export function ThemeManager({
  ...
  function renderDraftForm(onSubmit: () => void, submitLabel: string) {
    return (
      <div className="mt-2 space-y-2 rounded-lg border border-chabon/15 bg-koton p-3">
        <div className="grid gap-2 sm:grid-cols-3">
          <input autoFocus value={draft.labelFr} onChange={(e) => setDraft({ ...draft, labelFr: e.target.value })} placeholder="Libellé (FR) *" className="rounded-md border border-chabon/15 bg-white px-2 py-1.5 text-sm outline-none focus:border-liy" />
          <input value={draft.labelEn} onChange={(e) => setDraft({ ...draft, labelEn: e.target.value })} placeholder="Label (EN)" className="rounded-md border border-chabon/15 bg-white px-2 py-1.5 text-sm outline-none focus:border-liy" />
          <input value={draft.labelHt} onChange={(e) => setDraft({ ...draft, labelHt: e.target.value })} placeholder="Etikèt (HT)" className="rounded-md border border-chabon/15 bg-white px-2 py-1.5 text-sm outline-none focus:border-liy" />
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-ank/80">Couleur :</span>
          <button type="button" onClick={() => setDraft({ ...draft, color: null })} className={`h-5 w-5 rounded-full border ${draft.color === null ? 'border-liy ring-2 ring-chabon/30' : 'border-chabon/20'} bg-white`} title="Aucune" />
          {COLORS.map((c) => (
            <button key={c} type="button" onClick={() => setDraft({ ...draft, color: c })} className={`h-5 w-5 rounded-full ${draft.color === c ? 'ring-2 ring-offset-1 ring-chabon/40' : ''}`} style={{ backgroundColor: c }} title={c} />
          ))}
          <div className="ml-auto flex gap-2">
            <button type="button" disabled={busy || !draft.labelFr.trim()} onClick={onSubmit} className="rounded-md bg-chabon px-3 py-1.5 text-xs font-semibold text-koton transition hover:bg-chabon disabled:opacity-50">
              {submitLabel}
            </button>
            <button type="button" onClick={cancel} className="rounded-md border border-chabon/15 px-3 py-1.5 text-xs text-grafit hover:bg-koton">
              Annuler
            </button>
          </div>
        </div>
  ...
  function renderMoveForm(node: ThemeNode) {
    ...
    return (
      <div className="mt-2 space-y-2 rounded-lg border border-chabon/15 bg-koton p-3">
        <p className="text-xs text-grafit">
          Déplacer « <span className="font-medium text-ank">{node.labelFr}</span> » sous :
        </p>
        <div className="flex items-center gap-2">
          <select value={moveTarget} onChange={(e) => setMoveTarget(e.target.value)} className="flex-1 rounded-md border border-chabon/15 bg-white px-2 py-1.5 text-sm outline-none focus:border-liy">
          ...
          <button type="button" disabled={busy} onClick={() => doMove(node.id)} className="rounded-md bg-chabon px-3 py-1.5 text-xs font-semibold text-koton hover:bg-chabon disabled:opacity-50">
            Déplacer
          </button>
          <button type="button" onClick={cancel} className="rounded-md border border-chabon/15 px-3 py-1.5 text-xs text-grafit hover:bg-koton">
            Annuler
  ...
  function Row({ node, siblings, index, depth }: { node: ThemeNode; siblings: ThemeNode[]; index: number; depth: number }) {
    ...
    return (
      <li>
        <div className={`group flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-koton ${node.active ? '' : 'opacity-50'}`} style={{ paddingLeft: depth * 18 + 8 }}>
          <span className="h-3 w-3 shrink-0 rounded-full border border-chabon/15" style={{ backgroundColor: node.color ?? 'transparent' }} />
          <span className="text-sm font-medium text-ank">{label || node.labelFr}</span>
          <span className="font-mono text-[11px] text-ank/80">⟨{node.slug}⟩</span>
          {docs > 0 && <span className="rounded-full bg-pil px-1.5 text-[10px] text-grafit">{docs} doc</span>}
          {!node.active && <span className="rounded-full bg-chabon/10 px-1.5 text-[10px] text-grafit">archivé</span>}
          <span className="ml-auto flex items-center gap-1 opacity-0 transition group-hover:opacity-100">
            <button onClick={() => reorder(siblings, index, -1)} disabled={busy || index === 0} className="rounded px-1 text-ank/80 hover:bg-chabon/10 disabled:opacity-30" title="Monter">↑</button>
            <button onClick={() => reorder(siblings, index, 1)} disabled={busy || index === siblings.length - 1} className="rounded px-1 text-ank/80 hover:bg-chabon/10 disabled:opacity-30" title="Descendre">↓</button>
            <button onClick={() => startAdd(node.id)} className="rounded px-1.5 text-xs text-grafit hover:bg-chabon/10" title="Ajouter un sous-thème">+ sous-thème</button>
            <button onClick={() => startEdit(node)} className="rounded px-1.5 text-xs text-grafit hover:bg-chabon/10">Renommer</button>
            <button onClick={() => startMove(node)} className="rounded px-1.5 text-xs text-grafit hover:bg-chabon/10">Déplacer</button>
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="rounded-2xl border border-chabon/10 bg-white p-3">
        {tree.length === 0 ? (
          <p className="px-2 py-6 text-center text-sm text-ank/80">Aucun thème. Créez un premier domaine.</p>
        ) : (
          <ul>
            {tree.map((n, i) => (
