    ...
    <div className="min-h-screen bg-koton">
      <header className="border-b border-chabon/10 bg-chabon">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-4 py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href={`/${locale}/publications`} className="rounded-full border border-koton/30 px-4 py-1.5 text-sm font-medium text-koton hover:bg-white/10">
              ← Publications
            </Link>
          </div>
        </div>
      ...
      <div className="bg-chabon pb-12 pt-8 text-koton">
        <div className="mx-auto max-w-3xl px-4">
          <div className="mb-5 inline-flex overflow-hidden rounded-full border border-koton/30 font-mono text-xs">
            <Link href={base} className={`px-4 py-1.5 ${!isEn ? 'bg-chabon font-semibold text-koton' : 'text-koton/80 hover:bg-white/10'}`}>FR</Link>
            <Link href={`${base}?lang=en`} className={`px-4 py-1.5 ${isEn ? 'bg-chabon font-semibold text-koton' : 'text-koton/80 hover:bg-white/10'}`}>EN</Link>
          </div>
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-koton/70">{isEn ? pub.kickerEn : pub.kicker}</p>
          <h1 className="mt-3 font-serif text-3xl font-semibold leading-tight lg:text-[2.6rem]">{isEn ? pub.titleEn : pub.titleFr}</h1>
          <p className="mt-5 font-mono text-xs text-koton/70">
      ...
      <div className="mx-auto max-w-3xl px-4 py-12">
        ...
        <div className="mt-10 flex items-center gap-4 rounded-2xl border border-chabon/10 border-l-4 border-l-chabon bg-white p-5">
        ...
        <Link href={`/${locale}/publications`} className="mt-8 inline-block text-sm font-semibold text-chabon hover:underline">
      ...
      <footer className="border-t border-chabon/10 bg-white">
        <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-between gap-3 px-4 py-6 text-sm text-ank/80">
          ...
          <nav className="flex flex-wrap gap-x-5 gap-y-2">
            <Link className="hover:text-ank" href={`/${locale}/publications`}>Publications</Link>
