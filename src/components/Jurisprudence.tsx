              ...
              <ol className="space-y-3">
                {cases.map((c, i) => (
                  <li key={i} className="relative pl-8">
                    ...
                    {c.ref ? (
                      <>
                        <p className="text-sm font-semibold leading-snug text-lank"><CodeRefText text={c.ref} codeHrefs={codeHrefs} /></p>
                        {c.excerpt && <p className="mt-1 text-[13px] leading-relaxed text-lank/70"><CodeRefText text={c.excerpt} codeHrefs={codeHrefs} /></p>}
                      </>
                    ) : (
                      // Note sans intitulé distinct (Code civil) : l'extrait EST la note.
                      <p className="text-[13px] leading-relaxed text-lank/80"><CodeRefText text={c.excerpt ?? ''} codeHrefs={codeHrefs} /></p>
                    )}
                  </li>
                ))}
              </ol>
