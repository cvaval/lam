 ...
 *
 *     npx tsx scripts/porter-titre-ii-code-commerce.ts            # simulation
 *     npx tsx scripts/porter-titre-ii-code-commerce.ts --apply     # Me Vaval, elle seule
 *
 * ⚠️ LE STATUT DES ARTICLES 91/93/94/95 EST TRANCHÉ — § 13.11, décision de Me Vaval du
 * 26 août 2026, texte des articles à l'appui : « au lieu de remplacer, indique "amendé"
 * avec replis comme pour les autres modifications opérées au niveau de la loi. »
 * Il n'y a donc PLUS de drapeau `--statut` : ce script n'a qu'un comportement.
 *   · arts 91/93/94/95 → pastille « modifié » (libellé « amendé »), texte au corps INTACT,
 *     ancienne version en repli — SANS ArticleVersion propre : aucun texte nouveau n'existe
 *     sous ces numéros, en poser un serait une fabrication (le droit en vigueur vit sous
 *     1611-1/1611-2, où les renvois mènent) ;
 *   · art. 92 → pastille « abrogé » + ArticleVersion ABROGE — c'est le mot du décret ;
 *   · une variante « abrogé sur 91/93-95 » a existé pendant la mise au point : RETIRÉE,
 *     contraire à la décision consignée. Ne pas la réintroduire.
 *
 * ─── CE QUE LE SCRIPT FAIT (source de vérité : le corps de la fiche du décret, LU en base) ──
 *   · le Titre VI du Livre Ier reçoit ses DEUX CHAPITRES (art. 17 du décret, intitulés
 *     VERBATIM) : « Chapitre Premier — Du gage et du nantissement » (arts 1611-1 et 1611-2
 *     insérés, têtes « Art. N (D. du 14 mai 2020) ») et « Chapitre II — Des commissionnaires »
 *     (les sections existantes passent au niveau 5) ;
 *   · l'article 1611-1 REMPLACE l'article 91 ; le 1611-2 REMPLACE les articles 93 à 95 ;
 *     « L'actuel article 92 est abrogé » (art. 17 in fine) — pastille « abrogé » ;
 *   · les alinéas 3, 4 et 5 de l'article 600 (art. 18 du décret) s'AJOUTENT en fin de bloc :
 *     l'article n'en a que DEUX en base (mesuré le 26 août) — ils ne remplacent rien ;
 *   · les anciens 91-95 restent au corps, texte Vandal INTACT, avec pastille + ancienne
 *     version repliée + note connexe (patron Loi Statut du commerçant 2018, arts 1-8) ;
 *     les marques Vandal (« modifié » nus sur 92/93/94) sont REMPLACÉES, jamais empilées ;
 *   · § 7.9 : blocs connexe réciproques C.com↔décret (8 paires), bijection machine-vérifiée ;
 *   · re-clé : les clés `sec-K|art-N` sont LUES de segmentAnnotated, jamais écrites à la main
 *     (§ 6.3) — le placement des chapitres ENTRE l'en-tête du Titre VI et ses sections laisse
 *     TOUTES les clés existantes inchangées, avec CONTRE-ÉPREUVE (toc volontairement faux →
 *     la garde secs===toc.length parle).
 *
 * Préexistant signalé, non « réparé » (hors périmètre) : la clé de jurisprudence
 * sec-17|art-111 est inatteignable depuis la renumérotation Bail pro (111→1710-1).
 *
 * Remplace le brouillon de fondation scripts/data/decret-suretes/maj2026-ccom-apply.ts
 * (drapeau --variante=) : NE LANCER QUE CELUI-CI. État AVANT re-mesuré du 26 août :
 * scripts/data/decret-suretes/maj2026-ccom-avant.json (lecture seule).
 *
 * COORDINATION : ce script et scripts/maj-decret-suretes-2020.ts écrivent tous deux
 * l'annotationsJson du décret — les lancer SÉQUENTIELLEMENT, jamais en parallèle.
 */
...
// § 13.11 — TRANCHÉE le 26 août 2026 par Me Vaval : « amendé » avec replis. Le drapeau
// `--statut` de la mise au point est retiré ; s'il traîne dans une vieille commande, on
// REFUSE plutôt que d'ignorer en silence un choix que l'appelant croit encore ouvert.
if (process.argv.some((a) => a.startsWith('--statut=')))
  throw new Error(
    'Le drapeau --statut n’existe plus : la question § 13.11 a été tranchée le 26 août 2026 ' +
      '(« amendé » avec replis sur 91/93/94/95, « abrogé » sur 92). Relancer sans le drapeau.',
  )
const STATUT = 'modifie' as const
...
async function main() {
  ...
  console.log(`✓ cibles relues : Code de commerce « ${ccom.titleFr} » (${ccom.id}) · Décret « ${dec.titleFr} » (${dec.id}) · statut § 13.11 : amendé avec replis`)
  ...
  // Tolère la tête actuelle « Article 600.- » ET la tête rétablie « Article 600 alinéas 3, 4 et 5.- »
  // (§ 7.3, scripts/maj-decret-suretes-2020.ts) : les deux scripts se composent dans les deux ordres.
  ...
  // § 11.8 — aucune ancre dupliquée DANS la toc (collisions sec-*/sec-* incluses).
  if (new Set(newToc.map((t) => t.anchor)).size !== newToc.length) throw new Error('§ 11.8 — ancre dupliquée dans la toc — ANNULÉ')
  ...
  // § 11.8 — collision d'ancres sec-* / art-* : aucune ancre de toc ne doit être une ancre de corps.
  const collisions = newToc.map((t) => t.anchor).filter((a) => apresTexte.has(a))
  if (collisions.length) throw new Error(`§ 11.8 — collision d'ancre toc/corps : ${collisions.join(', ')} — ANNULÉ`)
  ...
  if (Array.isArray(ann.connexe)) throw new Error('connexe du C.com est un TABLEAU — les blocs ajoutés seraient perdus à la sérialisation (D5)')
  ...
  const statutRemplaces = 'modifié' // § 13.11 tranchée : « amendé » avec replis
  for (const n of OLD_REPLACED) ann.status[`art-${n}`] = statutRemplaces
  ...
  if (Array.isArray(annDec.connexe)) throw new Error('connexe du décret est un TABLEAU — les blocs ajoutés seraient perdus à la sérialisation (D5)')
  ...
  // Idempotence : seuls les blocs vers le C.com se remplacent — ceux vers le Code civil
  // (§ 7.4, scripts/maj-decret-suretes-2020.ts) ne sont pas touchés.
  ...
  console.log('  (l’art. 19 du décret — équivalence gage/nantissement — ne vise aucun article : aucun bloc, consigné)')

  // ── Plan d'ArticleVersion — § 13.11 TRANCHÉE (26 août 2026, « amendé » avec replis) ──
  // 91/93/94/95 : PAS d'ArticleVersion propre — aucun texte nouveau n'existe sous ces
  // numéros, en poser un serait une fabrication (le droit en vigueur vit sous 1611-1/1611-2).
  ...
  const plan: PlanAV[] = [plan600, plan92]
  ...
  console.log(`Marques Vandal remplacées (jamais empilées) : ${JSON.stringify(vandalAvant)} → 92=abrogé, 93/94=${statutRemplaces}`)
  console.log(`Statuts posés : 1611-1/1611-2=nouveau · 92=abrogé · 600=modifié · 91/93/94/95=${statutRemplaces} (§ 13.11, TRANCHÉE le 26 août 2026 : « amendé » avec replis)`)
  console.log('⚠ AFFICHAGE À SAVOIR (D2) : pour 91/93/94/95 le corps affiché reste l’ancien texte et le repli')
  console.log('  porte le même texte — contrairement au patron du Code civil où le repli montre l’ancien état')
  console.log('  SOUS un texte à jour. Ici l’article n’a pas de texte nouveau sous son numéro : le repli vaut')
  console.log('  photographie d’avant-réforme, la pastille et le renvoi vers 1611-1/-2 disent où vit le droit.')
  console.log('  À contrôler sur la fiche après --apply ; si Me Vaval préfère une note au lieu du repli, une ligne à changer.')
  console.log(`oldVersions : art-91/92/93/94/95/600 (texte Vandal verbatim, lignes « Nouveau » d'édition comprises — conservées, § 13 : purge non décidée)`)
  console.log(`Connexes C.com : 8 blocs · Connexes décret : ${PAIRS_CONNEXE.length} blocs (art-17 ×5, art-1611-1, art-1611-2, art-600)`)
  console.log(`ArticleVersion : ${plan.length} — 600 amendé + 92 ABROGE ; 91/93-95 sans AV propre (§ 13.11)`)
  console.log(`Typographie : textes portés VERBATIM du décret (apostrophes courbes), aucune normalisation (§ 6.6)`)

  if (!APPLY) {
    console.log('\nSIMULATION — rien n’a été écrit (base comme fichiers). --apply est réservé à Me Vaval.')
  ...
  const backupPath = join(DIR, `etat-anterieur-titre-ii-${stamp}.json`)
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        _lisezMoi: 'État AVANT scripts/porter-titre-ii-code-commerce.ts --apply (Code de commerce entier + annotations du décret).',
        statut: STATUT,
        ...
        codeCivil: { id: ccivAvant.id, md5BodyOriginal: ccivHash.body, md5AnnotationsJson: ccivHash.ann },
      },
      null,
      1,
    ),
    ...
    async (tx) => {
      ...
      await audit(
        { action: 'ARTICLE_AMENDED', targetType: 'Document', targetId: ccom.id,
          meta: { source: 'CODE_COMMERCE_ANNOTE', motif: `TITRE II du Décret sûretés porté (arts 1611-1/1611-2 créés, 92 abrogé, 91/93-95 ${statutRemplaces} avec replis, 600 al. 3-5) — § 13.11 tranchée le 26 août 2026 par Me Vaval`, articleVersions: plan.length, fichierEtatAnterieur: backupPath } },
        tx,
      )
      await audit(
        { action: 'ARTICLE_AMENDED', targetType: 'Document', targetId: dec.id,
          meta: { source: 'DECRET_SURETES', motif: `blocs connexes décret→Code de commerce (§ 7.9) : ${PAIRS_CONNEXE.length} paires`, fichierEtatAnterieur: backupPath } },
        tx,
      )
    },
