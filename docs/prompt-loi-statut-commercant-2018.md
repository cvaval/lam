...
- `SOURCE_PDF` = `/Users/cvaval/Library/CloudStorage/Dropbox/Moniteur/Statut-Commercant - 2018.pdf` (23 pages, **scanné → OCR obligatoire**, vérification page à page)
- **Périmètre dans le PDF : pages 10 à 23 UNIQUEMENT.** Les pages 1-9 contiennent la Loi N° 003-2018 (Formation Technique et Professionnelle), étrangère au sujet — à EXCLURE.
...
1. **Reproduction verbatim** de l'OCR vérifié — zéro reformulation. Le PDF est scanné : chaque article OCRisé est relu contre l'image de sa page avant intégration (pas d'OCR aveugle).
...
1. OCR des pages 10-23 (extraction du texte article par article, relecture contre l'image ; conventions : une ligne par alinéa, énumérations a)/1. conservées avec leur marqueur original).
2. `bodyOriginal` : bandeau du Moniteur retiré (provenance dans `moniteurRef`) ; intitulés multi-lignes joints « — » ; **pas de guillemets de citation** dans ce texte (structure directe, non guillemetée — vérifier sur pièce).
...
1. **Intitulé de la Section VI discordant** : le sommaire de l'article 1er dit « De l'**informatisation** du Registre du Commerce et du Fichier National » ; l'en-tête du corps (p. 20) dit « DE L'**INFORMATION** DU REGISTRE DU COMMERCE ET DU FICHIER NATIONAL ». Retenir l'en-tête du CORPS pour l'affichage (verbatim), consigner l'écart.
